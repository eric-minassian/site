...
export class AppStack extends cdk.Stack {
  ...
  constructor(scope: Construct, id: string, props: AppStackProps) {
    ...
    // ACM permissions for custom domain certificate management (us-east-1)
    ...
    // --- Custom Domain: wire KVS + distribution to API Lambda ---

    apiHandler.addEnvironment("SITES_KVS_ARN", sitesKvs.keyValueStoreArn);
    apiHandler.addEnvironment(
      "SITES_DISTRIBUTION_ID",
      sitesDistribution.distributionId,
    );

    // KVS permissions for API Lambda
    apiHandler.addToRolePolicy(
      new iam.PolicyStatement({
        actions: [
          "cloudfront-keyvaluestore:DescribeKeyValueStore",
    ...
    // CloudFront distribution update permissions for API Lambda
    apiHandler.addToRolePolicy(
      new iam.PolicyStatement({
        actions: [
    ...
    // --- Domain Cert Checker Lambda ---

    const domainCheckerHandler = new NodejsFunction(
      this,
      "DomainCheckerHandler",
      {
        ...
        environment: {
          SITES_TABLE: props.sitesTable.tableName,
          SITES_KVS_ARN: sitesKvs.keyValueStoreArn,
          SITES_DISTRIBUTION_ID: sitesDistribution.distributionId,
        },
        bundling: { minify: true, sourceMap: true },
      },
    );

    props.sitesTable.grantReadWriteData(domainCheckerHandler);

    domainCheckerHandler.addToRolePolicy(
      new iam.PolicyStatement({
    ...
    new events.Rule(this, "DomainCheckerSchedule", {
      schedule: events.Schedule.rate(cdk.Duration.minutes(5)),
      targets: [new targets.LambdaFunction(domainCheckerHandler)],
    });
    ...
    NagSuppressions.addResourceSuppressions(domainCheckerHandler, [
      {
        id: "AwsSolutions-L1",
        reason:
          "Using Node.js 22.x which is the latest supported Lambda runtime",
      },
    ]);
